# BrainShare — Product Specification v1.2
...
\[Goal: Public launch summer 2026\]
  ...
  \+-- requires \--\> \[Goal: Legit first impression\]
      ...
      |   \+-- addressed\_by \--\> \[Card: Implement SSO\]

      \+-- requires \--\> \[Goal: Safe stranger matching\]

      |   \+-- addressed\_by \--\> \[Card: Implement gamer tags\]
...
\[Decision: Bundle onboarding+SSO+tags\]
  ...
  \+-- proposed\_by \--\> \[Actor: Ziga\]

  \+-- approved\_by \--\> \[Actor: Will\]

  \+-- source \--\> \[Meeting: Sprint planning, Apr 22\]

  \+-- supersedes \--\> \[Prior Decision: Sequential build order\]

  \+-- depends\_on \--\> \[Assumption: Bundling is efficient\]

  \+-- depends\_on \--\> \[Assumption: SSO and tags are simple\]

  \+-- depends\_on \--\> \[Prerequisite: Marek's designs ready (validated)\]
  ...
  \+-- revisit\_when \--\> \[Trigger: Evidence bundling isn't efficient\]
...
- Decision → proposed\_by / approved\_by → Actor  
- Decision → source → Episode  
- Goal → requires → Goal (Why Chain hierarchy)  
- Goal → addressed\_by → Work Item  
- Work Item → owned\_by → Actor  
- Work Item → blocked\_by → Work Item  
- Standard → originated\_from → Decision  
- Signal → observed\_in → Episode  
- Signal → relates\_to → any primitive

Every relationship has a temporal dimension (valid\_from, valid\_to) inherited from Graphiti's temporal model.
...
- **Relationships:** depends\_on, supersedes, spawned, requires, addressed\_by, owned\_by, etc.  
- **Operational patterns:** decision\_decay, priority\_drift, ownership\_ambiguity, scope\_creep, etc.
...
| Tool integrations | Build custom \+ MCP | Product-specific write-back capability |
...
# 16\. What's Not Designed Yet

## Ingestion Pipeline

How BrainShare extracts typed primitives from raw tool data (Discord messages, Notion pages, Figma frames). The prompts, output formats, and confidence thresholds for extraction. Semantic matching between tools. **This is the recommended next design exercise: spec one end-to-end pipeline (e.g., Discord → Decision extraction) down to pseudocode.**
...
Mechanics of creating/updating in Notion, ClickUp, Slack, Figma. Conflict handling.
...
MCP-based connections to external tools. Authentication and maintenance.
...
*v1.2 changelog: Added Technical Architecture section (Section 15\) based on landscape research. Key decisions: build on Graphiti for temporal graph engine, differentiate through typed primitives, implement adaptive retrieval, consider ontology grounding for inborn knowledge, pre-structure causal context rather than asking LLMs to discover it. Added typed primitive definitions (Decision, Assumption, Goal, Actor, Work Item, Standard, Signal, Episode) with relationships. Added build-vs-build-on summary table. Updated open questions to reflect new architecture decisions and recommended next steps. Referenced context-memory-research-brief.md for full landscape analysis.*  
