-- Phase 1.6: field-level color + description + locked toggle.
-- Color moves from each option to the parent field — all option badges within
-- a field share the field's color, matching Factor's UX model.

alter table data_fields
  add column if not exists color text not null default 'badge-1',
  add column if not exists description text,
  add column if not exists locked boolean not null default false;

-- Backfill: adopt the first option's color as the field's color where available,
-- so the visual defaults roughly match what the user already has.
update data_fields f
set color = coalesce(
  (select color from data_field_options
     where field_id = f.id
     order by position asc
     limit 1),
  'badge-1'
)
where f.color = 'badge-1';

-- Options no longer carry color.
alter table data_field_options drop column if exists color;

-- Replace board RPC: field payload gains `color`; option payload loses `color`.
