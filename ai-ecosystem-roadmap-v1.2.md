...
- **2026-05-01 — Fractal attention scoping added to BrainShare onboarding.**
- Before any primitive extraction, BrainShare does a scoping pass — scans all connected sources and proposes a MECE categorization of the user's life/work domains with three-tier indicators (✅ full extraction, 🟡 lightweight tracking, ❌ ignore). User confirms via chat or toggles. The attention scope tree IS the skeleton of the Why Chain — it maps directly to WorkOS workspaces/stacks on graduation. Sub-categorization grows organically as BrainShare detects clusters.

- **2026-05-01 — Corrected extraction principle: conviction traces to human signal, not AI generation.**
- Both humans and AI produce content. But conviction always traces to human signal weighted by authority. An AI suggestion is not a decision until a human confirms it. A human's explicit statement carries conviction based on their authority in the relevant domain.
...
- **2026-05-01 — Memory Browser View added to WorkOS.**
- A "Memory" or "Context" view within WorkOS (alongside Board and Feed) — structured table/list of BrainShare primitives with filters by type, project, conviction, status, date, actor, source. Phase 2 feature.

- **2026-04-29 — BrainShare roadmap realigned to product spec v1.2 + extraction pipeline.**
- BrainShare is not "Claude chat in WorkOS plus integrations." It is the shared context engine underneath WorkOS/Swarm: BrainShare-first chat entry, cross-tool conversation continuity, Graphiti-backed temporal memory, typed primitive extraction, conviction scoring, WorkOS/external-tool writeback, and adaptive context assembly. Phase 2 now starts with the extraction pipeline and graph substrate, then layers WorkOS writeback and AI surfaces on top.
...
UI source of truth: Work OS platform/work-os-ui-design-spec.md. The design spec defines the layout shell, Board view, detail panel, design tokens, component hierarchy, and phase-annotated scope. This roadmap tracks build order and data/plumbing; the design spec tracks look and feel.
...
### 1.8 Saved Views — ✅ Complete

- [x] View tabs across the top of the board: each saved view stores column field; tab row with starred indicator, double-click rename, hover actions (star/rename/delete), [+] new view

- [x] Starred view is the workspace default (loaded on open); switching tabs loads that view's column_field_id; column field changes auto-save back to the active view

- [x] Toolbar: Filter (with active-count badge) — field/option picker popover; filters AND across fields, OR within a field; auto-saved to active view; new views clone current filters

- [x] Per-stack column-field override — each stack can use a different column field within a view; chosen via "Columns" section in stack QUAM; persists to workspace_views.stack_column_fields (migration 0011)

- [x] Column header collapse/expand — collapse button on header hover; collapsed columns render as 32px vertical strip with rotated name + count; click to re-expand; state persists per view

- [x] Per-workspace panel layout persistence — detail panel width saved to localStorage; restored on next load

- [x] Member/agent avatars on stack headers and cards — show owner avatar on board face; BoardAvatar component (initials circle, purple ring for agents); actors fetched by instance_id in board.ts and threaded through board → stack header + card tile
...
### 1.10.5 BrainShare Primitive Foundations + Planning Signals — ✅ Complete
...
- [ ] Card-level mirror display on board — mirror placement appears in the "Appears in" panel today, but mirrored cards are not yet shown on the board grid

- [ ] Mirror-aware move up/down for stack quick actions — drag/reorder is mirror-aware, but the stack QUAM move actions still need mirror-context handling

- [ ] Field-change log entries — postponed because every field action would need activity logging; revisit once BrainShare/Swarm need field history as context

- [ ] Post reactions (`post_reactions` table) — Phase 2 review/confirmation affordance, useful for BrainShare corrections and lightweight human signal

- [ ] Link-type swap (`related` ↔ `blocks`) without delete + recreate

- [ ] Workspace side panel / workspace posts design — defer until auth, membership management, and multi-user workspace semantics exist
...
Updated source of truth: apps/brainshare/context-docs/brainshare-product-spec-v1.3.md and apps/brainshare/context-docs/brainshare-extraction-pipeline.md. The older local-file/MCP build plan is superseded for roadmap purposes.
...
Core extraction principle: Both humans and AI produce content. But conviction always traces to human signal weighted by authority. An AI suggestion is not a decision until a human with authority confirms it.
...
- [x] Add BrainShare provider-key setup UX: collect Claude/OpenAI keys during onboarding or settings, store encrypted server-side, validate with a test call, and never expose secrets to browser/client code
...
- [x] Add CLI for power-user scripting (`brainshare query`, `brainshare ingest`)

### 2.1 Reference Extraction Pipeline: Claude/ChatGPT Conversations First
...
- [ ] Add first Why Chain model: goals → sub-goals → WorkOS nodes/cards, so any card at any nesting depth can answer "why does this exist?" — the attention scope tree from onboarding IS the skeleton of the Why Chain
...
- [ ] Auto-include linked context in BrainShare/AI invocations using explainable graph-distance rules, so related/blocking nodes can enter context without manual copying
...
- [ ] Connect-tools checklist scaffold: Claude/ChatGPT (first), Discord/Slack, PM tool, Figma, Google Drive/Docs, GitHub

**Fractal Attention Scoping (pre-step before any extraction):**

- [ ] BrainShare scans all connected sources and proposes a MECE categorization of the user's life/work domains in streaming "thinking out loud" style

- [ ] Presents three-tier indicators: ✅ (full extraction), 🟡 (lightweight tracking), ❌ (ignore) — with BrainShare's best guess pre-filled based on observed volume and importance

- [ ] User confirms via chat ("move fitness to green") OR toggles directly

- [ ] The attention scope tree IS the skeleton of the Why Chain — maps directly to WorkOS workspaces and stacks on graduation

- [ ] Sub-categorization grows organically as BrainShare detects clusters within scopes

- [ ] BrainShare proactively suggests new scopes over time: "You've been discussing meal planning a lot. Want me to start tracking that?"

- [ ] **Streaming cascading analysis:** first visible output within 2-3 seconds, then deeper tool-by-tool insights within confirmed attention scopes

- [ ] **Company Snapshot confirmation flow** — user corrects the Foundation layer conversationally

- [ ] **Context cleanup task side panel:** missing connections, stale context, alignment gaps, structural gaps, knowledge gaps — scoped to ✅ attention domains only

- [ ] Cross-tool conversation hop proof point: continue the same BrainShare conversation in Slack/Discord without re-explaining

### 2.7.5 Reference Extraction Pipeline: Discord Second
...
- [ ] Cross-source linking: connect Discord decisions to AI conversation decisions about the same topics
...
- [ ] Factor adapter: structured card/stack/field data ingestion — map to WorkOS nodes with BrainShare-inferred rationale and decisions from post streams

- [ ] External writeback patterns: Slack/Discord confirmations, Notion decision records, PM tool task updates, Google Docs decision logs — target cards OR stacks based on scope
...
Design source of truth: migration-design.md, updated by the BrainShare product spec's onboarding arc.

This is no longer "migration" as rote import. BrainShare first creates and maintains a context map across existing tools; WorkOS graduation happens when the user sees that BrainShare has already structured the work and chooses to operate there. The attention scope tree from onboarding IS the workspace/stack structure.
...
- [ ] Board reveal moment — "Want to see what this would look like in one place?" with the Board already populated and structured from the attention scope tree
